...
function parseInline(line) {
  ...
  while ((m = re.exec(line))) {
    ...
    const t = m[0];
    if (t.startsWith("**")) blocks.push({ tag: "b", text: t.slice(2, -2) });
    else if (t.startsWith("`")) blocks.push({ tag: "code", text: t.slice(1, -1) });
...
export function mdToFeishuPost(md) {
  ...
  for (const raw of lines) {
    ...
    if (line.trim().startsWith("```")) {
      if (!inCode) {
        ...
        inCode = false;
        content.push([{ tag: "code", text: codeBuf.join("\n") }]);
      }
      continue;
    }
    if (inCode) {
  ...
  if (inCode && codeBuf.length) content.push([{ tag: "code", text: codeBuf.join("\n") }]);
  return content;
}
